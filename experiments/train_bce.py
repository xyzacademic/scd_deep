...
filename = f'logs/{args.dataset}/{args.target}.csv'
...
trainset, testset = get_set(data=args.dataset, aug=args.aug, num_classes=args.n_classes, c0=args.c0, c1=args.c1)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=args.n_jobs, pin_memory=True)
test_loader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=args.n_jobs, pin_memory=True)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        outputs = net(data, layer=net.layers[-1]+'_projection').flatten().float()
        # print(outputs)
