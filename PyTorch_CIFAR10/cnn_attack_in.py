...
from tools import load_data, get_set
...
if __name__ == '__main__':
    ...
    source_list = [
        str(i) for i in range(args.votes)
    ]
    ...
    path = '../experiments/checkpoints/pt'
    source_model = args.source
    model_name = f'{args.dataset}_{args.c0}{args.c1}_{source_model}'
    checkpoints = [os.path.join(path, f'{model_name}_%d.pt' % i) for i in range(args.votes)]
    state_dict = [torch.load(checkpoints[i], map_location=torch.device('cpu')) for i in
                  range(len(checkpoints))]
    
    for i in range(args.votes):

        source_model = str(i)
        structure = arch[args.target]
        params = {
            'num_classes': 1,
            'votes': 1,
            'scale': 1,
        ...
        for target_model in range(args.votes):
            j = target_model
            ...
            try:
                ...
                for layer in layers:
                    if 'conv' in layer:
                        weights = torch.cat([state_dict[j][f'{layer}.weight']])
                        scd._modules[layer].weight = torch.nn.Parameter(weights,
                                                                        requires_grad=False)
                        try:
                            bias = torch.cat([state_dict[j][f'{layer}.bias']])
                            scd._modules[layer].bias = torch.nn.Parameter(bias, requires_grad=False)
                        except:
                            pass
                    elif 'fc' in layer:
                        weights = torch.cat(
                            [state_dict[j][f'{layer}.weight']]).unsqueeze(dim=1)
                        scd._modules[layer].weight = torch.nn.Parameter(weights,
                                                                        requires_grad=False)
                        try:
                            bias = torch.cat([state_dict[j][f'{layer}.bias']])
