...
for i in acts:
    for j in nbs:
        for k in dms:
            for m in seeds:
                if not os.path.exists(os.path.join('../checkpoints/pt', f'cifar10_fc_{i}_i1_bce_nb{j}_nw1_dm{k}_s1_fp32_32_{m}.pt')):
...
# for i, (version, scale) in enumerate(variables):
    ...
    part_3 = [
        ...
        'python train_cnn01.py --nrows 0.75 --localit 1 '
        ...
        f'--w-inc1 0.17 --w-inc2 0.2 --version fc --seed {params[3]} --iters 1000 '
        f'--dataset cifar10 --n_classes 2 --cnn 0 --divmean {params[2]} '
        f'--target cifar10_fc_{params[0]}_i1_bce_nb{params[1]}_nw1_dm{params[2]}_s1_fp32_32_{params[3]}.pkl '
