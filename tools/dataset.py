...
def get_set(data=None, num_classes=10, aug=1, c0=0, c1=1):
    """
    ...
    if data == 'mnist':
        ...
        testset = torchvision.datasets.CIFAR10(root=test_dir, train=False, download=True,
                                               transform=test_transform)
        train_idx = [True if i in [c0, c1] else False for i in trainset.targets]
        test_idx = [True if i in [c0, c1] else False for i in testset.targets]

        trainset.data = trainset.data[train_idx]
        testset.data = testset.data[test_idx]
        trainset.targets = [i for i in trainset.targets if i in [c0, c1]]
        testset.targets = [i for i in testset.targets if i in [c0, c1]]

        for i in range(len(trainset.targets)):
            trainset.targets[i] = 0 if trainset.targets[i] == c0 else 1
        for i in range(len(testset.targets)):
            testset.targets[i] = 0 if testset.targets[i] == c0 else 1

    elif data == 'stl10_binary':
        ...
        train_idx = [True if i in [c0, c1] else False for i in trainset.labels]
        test_idx = [True if i in [c0, c1] else False for i in testset.labels]

        trainset.data = trainset.data[train_idx]
        testset.data = testset.data[test_idx]
        trainset.labels = [i for i in trainset.labels if i in [c0, c1]]
        testset.labels = [i for i in testset.labels if i in [c0, c1]]

        for i in range(len(trainset.labels)):
            trainset.labels[i] = 0 if trainset.labels[i] == c0 else 1
        for i in range(len(testset.labels)):
            testset.labels[i] = 0 if testset.labels[i] == c0 else 1
        ...
        train_dir = '/home/y/yx277/research/ImageDataset/celeba/data/train_binary'
        test_dir = '/home/y/yx277/research/ImageDataset/celeba/data/test_binary'
        ...
        train_idx = [True if i < num_classes else False for i in trainset.targets]
        test_idx = [True if i < num_classes else False for i in testset.targets]
        trainset.imgs = [trainset.imgs[i] for i in range(len(train_idx)) if train_idx[i] is True]
        testset.imgs = [testset.imgs[i] for i in range(len(test_idx)) if test_idx[i] is True]
        trainset.targets = [i for i in trainset.targets if i < num_classes]
        testset.targets = [i for i in testset.targets if i < num_classes]

    elif data == 'gtsrb_binary':
        train_dir = '/home/y/yx277/research/ImageDataset/GTSRB/data/train_binary'
        test_dir = '/home/y/yx277/research/ImageDataset/GTSRB/data/test_binary'
