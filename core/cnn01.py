...
class toy4rrr100(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        ...
        self.conv2_si = nn.Conv2d(16, 32, kernel_size=3, padding=1, bias=bias)
        self.conv3_si = nn.Conv2d(32, 64, kernel_size=3, padding=1, bias=bias)
        self.conv4_si = nn.Conv2d(64, 128, kernel_size=3, padding=1, bias=bias)
        self.fc5_si = nn.Linear(4608, 100, bias=bias)
        self.fc6_si = nn.Linear(100, num_classes, bias=bias)
    ...
    def forward(self, x, input_=None, layer=None):
        ...
        if status < 5:
            ...
            if input_ == self.layers[4] + "_ap":
                out = x
            out = torch.relu_(out)
            if layer == self.layers[4] + "_output":
                return out

        # layer 6
        if input_ == self.layers[5]:
            out = x
        if status < 6:
            if input_ != self.layers[5] + "_ap":
                out = self.fc6_si(out)
            if layer == self.layers[5] + "_projection":
                return out
            if input_ == self.layers[5] + "_ap":
                out = x
            out = self.signb(out)
...
class toy3gssr100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(toy3gssr100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
        ...
        self.conv2_si = nn.Conv2d(16, 32, kernel_size=3, padding=1, bias=bias)
        self.conv3_si = nn.Conv2d(32, 64, kernel_size=3, padding=1, bias=bias)
        self.fc4_si = nn.Linear(2304, 100, bias=bias)
        self.fc5_si = nn.Linear(100, num_classes, bias=bias)
        self.layers = ["conv1_si", "conv2_si", "conv3_si", "fc4_si", "fc5_si"]
        self.apply(_weights_init)
...
class toy3gsss100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(toy3gsss100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class toy3gsrr100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(toy3gsrr100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class toy3grrr100(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(toy3grrr100, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class toy4ssss100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(toy4ssss100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class toy3gssss100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(toy3gssss100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class mlpcs20scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(mlpcs20scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
        ...
        self.fc1_si = nn.Linear(27648, 20, bias=bias)
        self.fc2_si = nn.Linear(20, num_classes, bias=bias)
        self.layers = ["fc1_si", "fc2_si"]
        self.apply(_weights_init)
...
class mlpcr20scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(mlpcr20scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class mlpcr100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(mlpcr100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
        ...
        self.fc1_si = nn.Linear(27648, 100, bias=bias)
        self.fc2_si = nn.Linear(100, num_classes, bias=bias)
        self.layers = ["fc1_si", "fc2_si"]
        self.apply(_weights_init)
...
class mlpcs100scale(nn.Module):
    def __init__(self, num_classes=2, act="sign", sigmoid=False, softmax=False, scale=1, bias=True):
        super(mlpcs100scale, self).__init__()
        if act == "sign":
            self.act = torch.sign
        elif act == "signb":
            self.act = signb
...
class mlp01scaleln(nn.Module):
    ...
    def forward(self, x, input_=None, layer=None):
        ...
        if status < 1:
            ...
            if input_ == self.layers[0] + "_ap":
                out = x
            out = msign((out-out.mean(dim=1, keepdims=True))/out.std(dim=1, keepdims=True)) * 0.2236
            if layer == self.layers[0] + "_output":
                return out
...
arch['mlp01scaleln'] = mlp01scaleln
...
arch['toy3gssr100scale'] = toy3gssr100scale
arch['toy3gsss100scale'] = toy3gsss100scale
arch['toy3gsrr100scale'] = toy3gsrr100scale
arch['toy3grrr100'] = toy3grrr100
arch['toy4ssss100scale'] = toy4ssss100scale
arch['toy3gssss100scale'] = toy3gssss100scale
arch['mlpcs20scale'] = mlpcs20scale
arch['mlpcr20scale'] = mlpcr20scale
arch['mlpcr100scale'] = mlpcr100scale
arch['mlpcs100scale'] = mlpcs100scale
