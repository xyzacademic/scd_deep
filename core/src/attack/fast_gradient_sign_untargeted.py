...
class FastGradientSignUntargeted():
    ...
    """
    def __init__(self, model, epsilon, alpha, min_val, max_val, max_iters,
                 _type='linf', loss_type='ce'):
        self.model = model
        # self.model.eval()
    ...
    def perturb(self, original_images, labels, reduction4loss='mean', random_start=False):
        ...
        if random_start:
            rand_perturb = torch.FloatTensor(original_images.shape).uniform_(
                max(-self.max_val, -self.epsilon), min(self.epsilon, self.max_val))
            rand_perturb = tensor2cuda(rand_perturb).type_as(original_images)
        ...
        with torch.enable_grad():
            for _iter in range(self.max_iters):
                ...
                if self.loss_type == 'ce':
                    ...
                    loss = F.nll_loss(outputs, labels, reduction=reduction4loss)
                elif self.loss_type == 'bce':
                    outputs = self.model(x, layer=self.model.layers[-1]+'_projection').flatten()
                    loss = F.binary_cross_entropy_with_logits(outputs, labels.float(), reduction=reduction4loss)
                ...
                x = project(x, original_images, self.epsilon, self._type).type_as(grads)
